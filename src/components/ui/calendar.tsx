...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      formatters={{
        formatMonthDropdown: (date) => date.toLocaleString(currentLanguage),
      ...
      components={{
        ...
        DayButton: (dayProps) => (
          <CalendarDayButton {...dayProps} events={events} />
        ),
...
function CalendarDayButton({
  ...
  const hasEvent = React.useMemo(() => {
    ...
    const dayFormatted = ptBRFormatterDate.format(new Date(day?.date));

    return events.some((event) => {
      const eventDate = event?.date;
      return eventDate === dayFormatted;
    });
  }, [day, events]);
  ...
  return (
    <>
      <Button
        ...
        variant="ghost"
        size="icon"
        data-day={day.date.toLocaleDateString(currentLanguage)}
      ...
      >
        ...
        {hasEvent && (
          <span
            className={
              'absolute -bottom-[-3px] h-1 w-4 rounded-lg bg-green-800'
            }
          />
        )}
      </Button>
